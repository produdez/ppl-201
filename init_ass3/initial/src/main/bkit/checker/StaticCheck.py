...
#TODO: re enable these line later 
from Visitor import *
from StaticError import *
#TODO: and disable these line
...
class StaticChecker(BaseVisitor):
    def __init__(self,ast):
        ...
        self.global_scope = [
Symbol("int_of_float",FunctionType([Symbol(None,FloatType(),Parameter())],IntType())),
Symbol("float_of_int",FunctionType([Symbol(None,IntType(),Parameter())],FloatType())),
Symbol("int_of_string",FunctionType([Symbol(None,StringType(),Parameter())],IntType())),
Symbol("string_of_int",FunctionType([Symbol(None,IntType(),Parameter())],StringType())),
Symbol("float_of_string",FunctionType([Symbol(None,StringType(),Parameter())],FloatType())),
Symbol("string_of_float",FunctionType([Symbol(None,FloatType(),Parameter())],StringType())),
Symbol("bool_of_string",FunctionType([Symbol(None,StringType(),Parameter())],BoolType())),
Symbol("string_of_bool",FunctionType([Symbol(None,BoolType(),Parameter())],StringType())),
Symbol("read",FunctionType([],StringType())),
Symbol("printLn",FunctionType([],VoidType())),
Symbol("print",FunctionType([Symbol(None,StringType(),Parameter())],VoidType())),
Symbol("printStrLn",FunctionType([Symbol(None,StringType(),Parameter())],VoidType()))]
    ...
    def visitCodeBlock(self,decl_ast,stmt_ast,env):
        ...
        env[0] = Helper.check_redeclare_and_expand(declarations,env[0])
    ...
    def visitProgram(self,ast, env):
        ...
        env[0] = Helper.check_redeclare_and_expand(global_decls,env[0])
        #second run on just the functions are needed
        Helper.checkEntryPoint(env)
    ...
    def visitVarDecl(self, ast:VarDecl, env):
        ...
        if ast.varDimen:
            ...
            return Symbol(var_name,var_type,Variable())
        
        #NOTE: vardecl is not check, just put the var-type to id
        #get var type
        var_type = ast.varInit.accept(self,(env,Unknown())) if ast.varInit else  Unknown()
    ...
    def visitFuncDecl(self, ast, env):
        ...
        if self.first_check:
            ...
            return Symbol(func_name,func_type, Function())
        ...
        current_env[0] = Helper.check_redeclare_and_expand(param_sym_list,current_env[0])
    ...
    def visitBinaryOp(self, ast:BinaryOp, params):
        env, infer_type = params
        op = ast.op
        op_type = self.op_type(op)
        left_type = ast.left.accept(self,(env,op_type))
        right_type = ast.right.accept(self,(env,op_type))

        if TypeCannotBeInferred in [left_type, right_type]:
    ...
    def visitUnaryOp(self, ast: UnaryOp, params):
        ...
        op = ast.op
        op_type = self.op_type(op)
        expr_type = ast.body.accept(self,(env,op_type))
    ...
    def visitCallExpr(self, ast: CallExpr, params):
        ...
        func_sym = Helper.symbol_lookup(func_name,env)
        ...
        #infer return type
        ret_type = Helper.infer(func_sym,infer_type)
        ...
        for param_sym,arg_ast in zip(param_sym_list,ast.param):
            atype = arg_ast.accept(self,(env,None))
            ptype = param_sym.returnType()
            # print('P: ',ptype,'A: ', atype)
            p_unknown = ptype == Unknown()
            a_unknown = atype == Unknown() or Helper.isUnknownArray(atype)

            if p_unknown and a_unknown:
                # print("Cant infer func-call-expr: ",param_sym,', Argument: ',arg_ast,'..with type: ',atype)
                return TypeCannotBeInferred
            if p_unknown:
                ptype = Helper.infer(param_sym,atype)
            if a_unknown:
                atype = arg_ast.accept(self,(env,ptype))
            if atype != ptype:
    ...
    def visitId(self, ast:Id, params):
        ...
        env, infer_type = params
        name = ast.name
        symbol = Helper.symbol_lookup(ast.name,env)
        if not symbol:
            # print('Undeclared',ast)
        ...
        #infer type here
        return Helper.infer(symbol,infer_type)
    ...
    def visitArrayCell(self, ast: ArrayCell, params):
        ...
        id_type = ast.arr.accept(self,(env,None))

        #NOTE: there's no way to infer dimension from array cell so error in this case
        if id_type == TypeCannotBeInferred or id_type == Unknown():
            # print('array-cell cant infer id', id_type)
            return TypeCannotBeInferred
        ...
        for i in idx:
            idx_expr_type = i.accept(self,(env,IntType()))
            
            if idx_expr_type == TypeCannotBeInferred:
                return TypeCannotBeInferred
    ...
    def visitAssign(self, ast: Assign, env):        
        ...
        right_type = ast.rhs.accept(self,(env,None))
        left_type = ast.lhs.accept(self,(env,None))
        ...
        left_unknown = left_type == Unknown() or Helper.isUnknownArray(left_type)
        right_unknown = right_type == Unknown() or Helper.isUnknownArray(right_type)

        if left_unknown and right_unknown:
            raise TypeCannotBeInferred(ast)
        if left_unknown:
            left_type = ast.lhs.accept(self,(env,right_type))
        if right_unknown: 
            right_type = ast.rhs.accept(self,(env,left_type))
    ...
    def visitIf(self, ast, env):
        ...
        for if_blk in ast.ifthenStmt:
            #check condition
            expr = if_blk[0]
            expr_type = expr.accept(self,(env,BoolType()))
            if expr_type == TypeCannotBeInferred:
                raise TypeCannotBeInferred(ast)
    ...
    def visitFor(self, ast, env):
        ...
        init_var_type = ast.idx1.accept(self,(env,IntType()))
        init_value_type = ast.expr1.accept(self,(env,IntType()))
        condition_type = ast.expr2.accept(self,(env,BoolType()))
        update_type = ast.expr3.accept(self,(env,IntType()))
        
        if TypeCannotBeInferred in [init_var_type, init_value_type,condition_type,update_type]:
            raise TypeCannotBeInferred(ast)
    ...
    def visitReturn(self, ast:Return, env):
        ...
        expr_type = ast.expr.accept(self,(env,None)) if ast.expr else VoidType() #NOTE: infer expression
        
        if expr_type == TypeCannotBeInferred:
            raise TypeCannotBeInferred(ast)

        return_unknown = func_rtype == Unknown()
        expr_unknown = expr_type == Unknown() or Helper.isUnknownArray(expr_type)
        ...
        if expr_unknown:
            #infer expr 
            expr_type = ast.expr.accept(self,(env,func_rtype))
        if return_unknown:
            func_rtype = Helper.infer(func_symbol,expr_type)
    ...
    def visitDowhile(self, ast, env):
        ...
        condition_type = ast.exp.accept(self,(env,BoolType()))
        
        if condition_type == TypeCannotBeInferred:
            raise TypeCannotBeInferred(ast)
    ...
    def visitCallStmt(self, ast:CallStmt, env):
        ...
        #infer return type
        func_rtype = Helper.infer(func_sym,VoidType())
        ...
        for param_sym,arg_ast in zip(param_sym_list,ast.param):
            ...
            if atype == TypeCannotBeInferred:
                raise TypeCannotBeInferred(ast)
            
            p_unknown = ptype == Unknown() #param can only be unknown, not unknown array.
            a_unknown = atype == Unknown() or Helper.isUnknownArray(atype)
    ...
    def visitArrayLiteral(self, ast, params):
        env,infer_type = params
        elems = ast.value
        elem_type = elems[0].accept(self,(env,Unknown()))

        if elem_type == TypeCannotBeInferred :
            return TypeCannotBeInferred
