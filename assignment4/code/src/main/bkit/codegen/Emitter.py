...
class Emitter():
    ...
    def getJVMType(self, inType):
        ...
        if typeIn is cgen.IntType:
            ...
            return "(" + "".join(list(map(lambda x: self.getJVMType(x.mtype), inType.partype))) + ")" + self.getJVMType(inType.rettype)
    ...
    *   @param in the code to be printed out
    ...
    def emitNEWARRAY(self,inType,frame):
        frame.push()
        if type(inType.eleType) != cgen.ArrayType and type(inType.eleType) != cgen.StringType:
            lexeme = self.getFullType(inType.eleType)
